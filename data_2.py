...
def load_data(args, tsne=False):
    ...
    if args.dataset.startswith('cifar'):
        ...
        if args.dataset == 'cifar100':
            train_dataset = datasets.CIFAR100(args.data_dir, train=True, download=args.download, transform=train_transform)
...
def load_resized_data(args):
    ...
    if args.dataset == 'cifar10':
        ...
        transform_test = transforms.Compose([transforms.ToTensor(), normalize])
        val_dataset = datasets.CIFAR100(args.data_dir, train=False, download=args.download, transform=transform_test)
        ...
        train_dataset = datasets.MNIST('./datasets', train=True, transform=transforms.ToTensor(), download=args.download)

        normalize = transforms.Normalize(mean=MEANS['mnist'], std=STDS['mnist'])
        transform_test = transforms.Compose([transforms.ToTensor(), normalize])

        val_dataset = datasets.MNIST('./datasets', train=False, transform=transform_test, download=args.download)
        train_dataset.nclass = 10
